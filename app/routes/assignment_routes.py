...
@assignment.route('/assignments/<int:assignment_id>/row')
@login_required
def assignment_row(assignment_id):
    # Return a HTML fragment containing a single <tr> (rendered via assignments_table partial)
    assignment_q = (
        db.session.query(
            Assignment,
            Class.class_name,
        ...
        .join(Class, Assignment.class_id == Class.class_id)
        .filter(Class.user_id == current_user.user_id, Assignment.assignment_id == assignment_id)
        .first()
    )

    if not assignment_q:
        print("You deleted the assignment already")
        abort(404)

    a, class_name, class_importance, class_type, class_is_finished, created_at, study_minutes = assignment_q
    ...
    return render_template('partials/assignments_table.html', rows=[row], show_class_column=True)


@assignment.route("/assignments/table")
@login_required
def assignments_table():
    # Support optional class-specific table render for partial refreshes
    ...
    if class_id:
        assignments = (
            ...
            .join(Class, Assignment.class_id == Class.class_id)
            .filter(Class.user_id == current_user.user_id, Assignment.class_id == class_id)
            .order_by(Assignment.due_at.asc())
            .all()
        )
        ...
        return render_template('partials/assignments_table.html', rows=rows, show_class_column=False)

    # Default: full table render
    assignments = (
        db.session.query(
            Assignment,
            Class.class_name,
    ...
    return render_template('partials/assignments_table.html', rows=rows, show_class_column=show_class_column)
