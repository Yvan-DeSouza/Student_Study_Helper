...
@selector.route("/api/select/classes2", methods=["POST"])
@login_required
def select_classes_v2():
    ...
    class_ids = [c.class_id for c in query.all()]
...
@selector.route("/api/select/assignments2", methods=["POST"])
@login_required
def select_assignments_v2():
    data = request.get_json() or {}
    ...
    for a in assignments:
        classes.setdefault(a.class_id, []).append(a.assignment_id)
    ...
    return jsonify({
        ...
        "classes": [
            ...
            for class_id, assignment_ids in classes.items()
        ],
    }), 200
