...
from app.services.analytics.pipelines.risk_pipeline import (
    build_assignment_dataframe,
    compute_assignment_risk_pipeline
)
...
def risk_composition_evolution():
    ...
    df = build_assignment_dataframe(results, past_assignments)
...
def assignment_risk_breakdown():
    ...
    df = compute_assignment_risk_pipeline(df, now, past_risk_assignments)
    if mode == 'riskiest':
